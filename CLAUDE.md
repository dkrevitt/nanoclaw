...
docker build --no-cache -t nanoclaw-agent .  # Force rebuild without cache
```

Verify after rebuild: `docker run -i --rm --entrypoint wc nanoclaw-agent:latest -l /app/src/index.ts`

## Deployment (DigitalOcean)

```bash
./deploy.sh                       # Build and start on VPS
